...
- Google account selection stored as one email in Preferences DataStore and Gmail `gmail.modify` access via Google credentials.
...
- Room database v2 with account, legacy per-message, and conversation-snapshot tables; an explicit 1-to-2 migration exists.
...
- Gmail backup is a manual test action capped at three changed conversations; it has progress, bounded error summaries, and no durable retry/resume.
- Account-shaped Room data exists, but the app selects and stores only one active Gmail email. Settings disconnect clears local DataStore only; OAuth revocation is empty.
